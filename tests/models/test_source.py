...
def test_cover_image_path_uses_default_cover_image(db: Session) -> None:
    ...
    db.add(source)
    db.commit()
    data = db.get_one(Source, source.id, populate_existing=False)

    assert data.cover_image_file is None
    assert data.cover_image_path.endswith("/images/default_cover_image.png")
